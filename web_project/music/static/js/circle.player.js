...
var CirclePlayer = function(jPlayerSelector, media, options) {
	var self = this,
		defaults = {
			...
			supplied: "mp3, wav, webma, fla, rtmpa, m4a, oga",
...
CirclePlayer.prototype = {
	...
	_initPlayer: function() {
		...
		this.player.bind($.jPlayer.event.ready + this.eventNamespace, function(
			event
		) {
		...
		this.player.bind($.jPlayer.event.play + this.eventNamespace, function(
			event
		) {
		...
		this.player.bind($.jPlayer.event.timeupdate + this.eventNamespace, function(
			event
		) {
			if (!self.dragging) {
		...
		this.player.bind($.jPlayer.event.progress + this.eventNamespace, function(
			event
		) {
			var percent = 0;
			if (
				typeof self.audio.buffered === "object" &&
				self.audio.buffered.length > 0
			) {
				if (self.audio.duration > 0) {
					var bufferTime = 0;
					for (var i = 0; i < self.audio.buffered.length; i++) {
						bufferTime +=
							self.audio.buffered.end(i) - self.audio.buffered.start(i);
				...
				// Fallback if buffered not supported
		...
		this.player.bind($.jPlayer.event.ended + this.eventNamespace, function(
			event
		) {
			self._resetSolution();
		});
	...
	_timeupdate: function(percent) {
		...
		var spriteOffset =
			(Math.floor(Math.round(percent) * this.spriteRatio) - 1) *
			-this.spritePitch;
